...
void ViewportOrtho::renderViewport(const RenderOptions &options) {
    ...
    if (dstPos) {
        ...
        const auto srcSvx = readVoxel(srcPos);
...
void ViewportOrtho::renderBrush(const Coordinate coord) {
    ...
    const auto move_condition = !bminvalid(Annotation::singleton().annotationMode.testFlag(AnnotationMode::SubObjectSplit), seg.startId, seg.hovered_subobject_id);
